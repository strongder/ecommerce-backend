        ...
        return cartItems.stream().map(
                cartItem -> {
                    if (!cartItem.isDelete()) {
                        double price = cartItem.getVarProduct().getProduct().getPrice();
                        double discount = cartItem.getVarProduct().getProduct().getDiscount();
                        double quantity = cartItem.getQuantity();
                        return price * (1 - discount / 100) * quantity;
                    } else {
                        return 0.0; // Trả về 0.0 nếu sản phẩm đã bị đánh dấu xóa
                    }
                }
    ...
    public void clearCart(Long id) {
        ...
        for (CartItem cartItem : list) {
            if (!cartItem.isDelete()) {
                ...
                variantProduct.setQuantitySold(variantProduct.getQuantitySold() + cartItem.getQuantity());
                Product product = productRepository.findByVarProductsId(variantProduct.getId());
